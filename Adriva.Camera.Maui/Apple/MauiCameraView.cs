...
namespace Adriva.Camera.Maui.Platforms.Apple;
...
public class MauiCameraView : UIView, IAVCaptureVideoDataOutputSampleBufferDelegate, IAVCaptureFileOutputRecordingDelegate, IAVCapturePhotoCaptureDelegate
{
    ...
    public MauiCameraView(CameraView cameraView)
    {
        ...
        this.PreviewLayer = new(captureSession)
        {
            ...
            if (await CameraView.RequestPermissions(true))
            {
                if (cameraView.Camera != null && cameraView.Microphone != null && captureSession != null)
                {
                    try
                    {
                        ...
                        movieFileOutputConnection.VideoOrientation = DisplayOrientation.Landscape == this.cameraView.Orientation ? AVCaptureVideoOrientation.LandscapeLeft : AVCaptureVideoOrientation.Portrait;
    ...
    public UIImage CropImage(UIImage originalImage)
    {
        ...
        UIGraphicsImageRenderer renderer = new UIGraphicsImageRenderer(originalImage.Size, new()
        {
            Opaque = false,
            Scale = 1
        });
    ...
    void DidFinishProcessingPhoto(AVCapturePhotoOutput captureOutput, AVCapturePhoto capturePhoto, NSError error)
    {
        if (null == capturePhoto || null != error)
        {
            photoError = true;
            return;
        }

        photo = new UIImage(capturePhoto.FileDataRepresentation);
        photoTaken = true;
    }
