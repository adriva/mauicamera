using Adriva.Camera.Maui.ZXingHelper;
...
public class CameraView : View, ICameraView
{
    ...
    public static readonly BindableProperty OrientationProperty = BindableProperty.Create(nameof(Orientation), typeof(DisplayOrientation), typeof(CameraView), DisplayOrientation.Portrait);
    ...
    {
        get => (DisplayOrientation)this.GetValue(CameraView.OrientationProperty);
        set => this.SetValue(CameraView.OrientationProperty, value);
    }
    ...
    public async Task<bool> SaveSnapShot(ImageFormat imageFormat, string SnapFilePath)
    {
        ...
        if (Handler != null && Handler is CameraViewHandler handler)
        {
            result = await handler.SaveSnapShotAsync(imageFormat, SnapFilePath);
        }
        return result;
    }
    /// <summary>
